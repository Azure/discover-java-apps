...
type CliApp struct {
	...
	JvmMemoryInMB     int       `json:"jvmMemoryInMB" csv:"HeapMemory(MB)"`
	...
	JarSize           int64     `json:"jarSize" csv:"JarFileSize(Bytes)"`
	LastModifiedTime  time.Time `json:"lastModifiedTime" csv:"JarFileModifiedTime"`
}
...
func (s springBootAppConverter) Convert(apps []*springboot.SpringBootApp) []*CliApp {
	...
	for _, app := range apps {
		...
		results = append(results, &CliApp{
			...
			JarSize:           app.JarSize,
			JvmMemoryInMB:     app.Runtime.JvmMemoryInMB,
